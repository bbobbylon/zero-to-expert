---
title: "Survival"
description: "Survival and living off the land, making essentials from scratch (paper, cheesecloth, pencils, bowls, blankets), and survival medical skills."
domain: survival
pageType: overview
status: planned
safetyCritical: true
sidebar:
  label: Overview
  order: 0
---

Survival and living off the land, making essentials from scratch (paper, cheesecloth, pencils, bowls, blankets), and survival medical skills.

## Roadmap

| Level | Name | You can... |
| --- | --- | --- |
| 0 | Orientation | Name the parts, tools, and vocabulary, and know the safety basics. |
| 1 | Beginner | Complete common jobs by following a walkthrough. |
| 2 | Intermediate | Do routine work on your own and spot mistakes. |
| 3 | Advanced | Take on multi-part jobs and diagnose problems from symptoms. |
| 4 | Expert | Do professional-grade work. |
| 5 | Mastery | Handle rare and complex work, and explain why things work the way they do. |

Each level page links to its walkthroughs. Once written, the cheat sheet will list common, uncommon, and rare procedures.

See [how these guides work](/about/how-guides-work/) for what the levels, tiers, and page statuses mean.
