---
title: "AWS infrastructure"
description: "Building and running infrastructure on AWS, from account setup to production architecture."
domain: aws
pageType: overview
status: draft
safetyCritical: false
sidebar:
  label: Overview
  order: 0
---

Building and running infrastructure on AWS, from account setup to production architecture.

## Roadmap

| Level | Name | You can... |
| --- | --- | --- |
| 0 | Orientation | Name the parts, tools, and vocabulary, and know the safety basics. |
| 1 | Beginner | Complete common jobs by following a walkthrough. |
| 2 | Intermediate | Do routine work on your own and spot mistakes. |
| 3 | Advanced | Take on multi-part jobs and diagnose problems from symptoms. |
| 4 | Expert | Do professional-grade work. |
| 5 | Mastery | Handle rare and complex work, and explain why things work the way they do. |

**Start here:** [Level 0: Orientation](/aws/levels/0-orientation/). It links five concept pages, six walkthroughs, and the [CLI cheat sheet](/aws/cheat-sheet/).

Each level page links to its walkthroughs. Once written, the cheat sheet will list common, uncommon, and rare commands.

See [how these guides work](/about/how-guides-work/) for what the levels, tiers, and page statuses mean.
